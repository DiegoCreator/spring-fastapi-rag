...
def test_vector_similarity_search(pg_db: Session):
    ...
    vecB = DocumentChunk(content="Oil painting on canvas", embedding=[0.1] * 384)
    try:
        pg_db.add_all([vecA, vecB])

        vec_query = [0.85] * 384
        results = pg_db.query(DocumentChunk).order_by(DocumentChunk.embedding.l2_distance(vec_query)).limit(1).all()

        assert len(results) == 1
        assert "psyhics" in results[0].content
    finally:
        pg_db.query(DocumentChunk).delete()
        pg_db.commit()
